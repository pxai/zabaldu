...
$votes_freq = intval($db->get_var("select count(*) from votes where vote_type='comments' and vote_user_id=$current_user->user_id and vote_date > subtime(now(), '0:0:30') and vote_ip = '".$globals['user_ip']."'"));
...
$votes_info = $db->get_row("select comment_user_id, comment_votes, comment_karma from comments where comment_id=$id");
...
echo "$votes_info->comment_votes,$votes_info->comment_karma";
