...
$now = time();
$max_date = "date_sub(now(), interval 20 minute)";
...
$negatives = $db->get_results("select SQL_NO_CACHE link_id, link_karma, link_votes, link_negatives from links where link_date < $max_date and link_date > $min_date and link_status = 'queued' and link_karma < 50 and link_negatives > 3 order by link_negatives desc limit 10 ");
