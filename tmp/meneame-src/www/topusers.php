...
if ($users) {
	foreach($users as $dbuser) {
		...
		$user->all_stats();
		echo '<tr>';
		echo '<td><a href="'.$globals['base_url'].'user.php?login='.$user->username.'"><img src="'.get_avatar_url($user->id, $user->avatar, $user->email, 20).'" width="20" height="20" alt="avatar" />'.$user->username.'</a></td>';
