...
if ($dbusers) {
	...
	foreach($dbusers as $dbuser) {
		...
		echo '</div>';
		echo '<div class="item">';
		echo '<a href="user.php?login='.urlencode($friend->username).'" title="'.$friend->username.'">';
		echo '<img src="'.get_avatar_url($friend->id, $friend->avatar, $friend->email, 20).'" width="20" height="20" alt="'.$friend->username.'"/>';
