...
function get_votes($time) {
	...
	$res = $db->get_results("select vote_id, unix_timestamp(vote_date) as timestamp, vote_value, vote_ip, vote_user_id, link_id, link_title, link_uri, link_status, link_date, link_published_date, link_votes, link_author from votes, links where vote_type='links' and vote_date > from_unixtime($time) and link_id = vote_link_id $pvotes and vote_user_id != link_author order by vote_date desc limit $max_items");
	if (!$res) return;
	foreach ($res as $event) {
		...
		$comments = $db->get_var("select count(*) from comments where comment_link_id = $event->link_id");
		$id=$event->vote_id;
		...
		$events[$key] = 'ts:"'.$event->timestamp.'",type:"'.$type.'",votes:"'.$event->link_votes.'", com:"'.$comments.'",link:"'.$link.'",title:"'.addslashes($event->link_title).'",who:"'.addslashes($who).'",status:"'.$status.'",uid:"'.$uid.'",md5:"'.$md5.'"';
		//echo "($key)". $events[$key];
...
function get_comments($time) {
	global $db, $events, $last_timestamp, $foo_link, $max_items;
	$res = $db->get_results("select comment_id, unix_timestamp(comment_date) as timestamp, user_login, user_email, comment_user_id, link_author, link_id, link_title, link_uri, link_status, link_date, link_published_date, link_votes from comments, links, users where comment_date > from_unixtime($time) and link_id = comment_link_id and link_votes > 0 and user_id=comment_user_id order by comment_date desc limit $max_items");
	if (!$res) return;
	foreach ($res as $event) {
		$foo_link->id=$event->link_id;
		$foo_link->uri=$event->link_uri;
		...
		$events[$key] = 'ts:"'.$event->timestamp.'",type:"'.$type.'",votes:"'.$event->link_votes.'",com:"'.$comments.'",link:"'.$link.'",title:"'.addslashes($event->link_title).'",who:"'.addslashes($who).'",status:"'.$status.'",uid:"'.$uid.'",md5:"'.$md5.'",cid:"'.$id.'"';
		//echo "($key)". $events[$key];
		if($event->timestamp > $last_timestamp) $last_timestamp = $event->timestamp;
	}
}
